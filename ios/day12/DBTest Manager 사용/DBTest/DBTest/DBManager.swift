...
class DBManager{
    ...
    //행 데이터 삽입
    func doInsert(_ insertValue: String){
        var insertStmt: OpaquePointer?
        
        if sqlite3_prepare_v2(db, "insert into \(TABLE_NAME) values (null, ?)", -1, &insertStmt, nil) == SQLITE_OK{
            let SQLITE_TRANSIENT = unsafeBitCast(-1, to: sqlite3_destructor_type.self)
            
            if sqlite3_bind_text(insertStmt, 1, insertValue, -1, SQLITE_TRANSIENT) != SQLITE_OK{
    ...
    //데이터 조회
    func doSelectAll(_ findCondition: String){
        
        let sql = "select * from \(TABLE_NAME) "//where \(COL_NAME) = '\(findCondition)'"
